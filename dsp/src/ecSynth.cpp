...
void ecSynth::init(al::AudioIOData *io) {
  ...
  panLFO.registerChangeCallback([&](int value) { pan.setModulationSource(Modulators[value]); });
...
void ecSynth::onProcess(AudioIOData &io) {
  ...
  while (io()) {
    ...
    if (modGrainRateDepth.getParam() > 0)  // modulate the grain rate
      grainScheduler.setFrequency(grainRate.getModParam(modGrainRateDepth.getParam()));
    else
    ...
    if (modIntermittencyDepth.getParam() > 0)  // modulate the intermittency
      grainScheduler.setIntermittence(intermittency.getModParam(modIntermittencyDepth.getParam()));
    else
      grainScheduler.setIntermittence(intermittency.getParam());

    if (modStreamsDepth.getParam() > 0)  // Modulate the amount of streams playing.
      grainScheduler.setPolyStream(consts::synchronous,
                                   streams.getModParam(modStreamsDepth.getParam()));
    else
...
bool ecSynth::removeCurrentSoundFile() { removeSoundFile(soundFile.mParameterInt->get() - 1); }
...
void ecSynth::resampleSoundFiles() {
  ...
  clearSoundFiles();
  for (long unsigned i = 0; i < filePaths.size(); i++) loadSoundFile(filePaths[i]);
}
