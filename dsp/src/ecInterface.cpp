...
void ecInterface::onCreate() {
    ...
    mPresets
        ...
        << *granulator.soundFile.mParameterInt
        << *granulator.soundFile.mLowRange << *granulator.soundFile.mHighRange
        << granulator.soundFileLFO << *granulator.modSoundFileDepth.mParameter
        ...
        << *granulator.modSawWidth.mParameter
        << *granulator.modSawWidth.mLowRange
...
void ecInterface::onDraw(Graphics &g) {
    ...
    // Draw Scope window
    ParameterGUI::beginPanel("Scope", windowWidth / 4,
    ...
    ImGui::PlotLines("Scope", audioIO().outBuffer(0), consts::BLOCK_SIZE, 0,
                     nullptr, FLT_MAX, FLT_MAX, ImVec2(0, 120),
                     2 * sizeof(float));
    ImGui::PopItemWidth();
    ...
    if (ImGui::CollapsingHeader("Record Audio",
        ...
        if (recordButtonClicked) {
            ...
            if (state.recordButton) {
                ...
                if (!recorder->start(directory + filename, frameRate,
                                     numChannels, ringBufferSize)) {
