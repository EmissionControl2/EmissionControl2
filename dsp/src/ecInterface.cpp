...
void ecInterface::onCreate() {
    ...
    ImGui::GetIO().Fonts->AddFontFromFileTTF(
        (f.directory(execPath) + "../Resources/Fonts/Roboto-Medium.ttf").c_str(), 14.0f);
#endif
...
void ecInterface::onDraw(Graphics &g) {
    ...
    drawRecorderWidget(&mRecorder, audioIO().framesPerSecond(), audioIO().channelsOut(),
                       soundOutput);
    ParameterGUI::endPanel();
    ...
    ParameterGUI::beginPanel("File Selector", 0, windowHeight * 3 / 4 + 25, windowWidth / 4,
